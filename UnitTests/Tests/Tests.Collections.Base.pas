...
function TTestCollectionBase<T>.Comparable: Boolean;
var
  TypeName: String;
begin
  { Check what types make sense to compare to each other (for sorting and
    binary search) }
  case FTypeInfo.Kind of
    ...
    tkRecord:
      begin
        { Managed records contain varying pointers }
        {$IFDEF NEXTGEN}
        TypeName := FTypeInfo.NameFld.ToString;
        {$ELSE}
        TypeName := String(FTypeInfo.Name);
        {$ENDIF}
        Result := (TypeName <> 'TManagedRecord') and (TypeName <> 'TFooBarRecord');
      end;

    tkDynArray:
      Result := not (FTypeData.DynArrElType^^.Kind in [tkUString, tkRecord]);
  else
    Result := True;
  end;
end;
...
begin
  case FTypeInfo.Kind of
    ...
    tkClass:
      begin
        System.Assert(System.TypeInfo(T) = System.TypeInfo(TFoo));
